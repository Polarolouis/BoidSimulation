...
alignment_checkbox = tkinter.Checkbutton(forces_frame, text="Alignment", font=("Helvetica", 16), variable=alignment_boolean, onvalue=True, offvalue=False)
if ALIGNMENT:
...
cohesion_checkbox = tkinter.Checkbutton(forces_frame, text="Cohesion", font=("Helvetica", 16), variable=cohesion_boolean, onvalue=True, offvalue=False)
if COHESION:
...
separation_checkbox = tkinter.Checkbutton(forces_frame, text="Separation", font=("Helvetica", 16), variable=separation_boolean, onvalue=True, offvalue=False)
if SEPARATION:
...
wind_checkbox = tkinter.Checkbutton(forces_frame, text="Wind", font=("Helvetica", 16), variable=wind_boolean, onvalue=True, offvalue=False, command=toggle_wind_parameters)
if WIND:
...
wind_direction_slider = tkinter.Scale(wind_parameters_frame, from_=0, to=360, orient=tkinter.HORIZONTAL, 
    length=200, label="Wind direction", font=("Helvetica", 16))
...
def validate_parameters():
    """Set the parameters"""
    global NUMBER_OF_BOIDS, NUMBER_OF_STEPS, BOUNCING, ALIGNMENT, SEPARATION, COHESION, WIND, WIND_SPEED, WIND_DIRECTION
...
def set_recap_labels():
    ...
    global label_bouncing, BOUNCING, label_alignment_forces, ALIGNMENT, label_cohesion_forces, COHESION, \
...
def start_simulation(root, number_of_boids, width, height):
    ...
    simulation_space.populate(number_of_boids, bouncing=BOUNCING, 
        alignment_bool=ALIGNMENT, cohesion_bool=COHESION, separation_bool=SEPARATION, wind_bool=WIND, wind_speed=WIND_SPEED, wind_direction=WIND_DIRECTION)
    # Start the simulation
